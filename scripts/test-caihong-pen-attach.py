...
struct pen_power {
    ...
    u32 valid;
    u16 values[8];
    bool tx_requested, cycle_requested;
};
...
{ if (scenario == 20 && current->exchange.enabled && g == current->scan) return 0; return g->value; }
...
static bool queue_delayed_work(int queue, struct delayed_work *w, unsigned long delay)
{
    ...
    assert(delay == PEN_ATTACH_MS && !w->queued);
...
static unsigned long wait_for_completion_timeout(struct completion *c, unsigned long ms)
{
    ...
    jiffies += ms;
    if (scenario == 19) current->lost.done = true;
...
static int i2c_transfer(void *adapter, struct i2c_msg *m, int n)
{
    ...
    if (reg == 9 || reg == 10) {
        ...
        if (reg == 10 && current->exchange.enabled) event_index++;
...
static int pen_read(struct pen_power *p, u16 reg, unsigned int len, u16 *value)
{
    ...
    if (reg == 7 && p->exchange.enabled) {
        if (scenario == 10) return -EIO;
        if (scenario == 11) { *value = PEN_INT_REMOVE; return 0; }
        if (scenario == 12) { *value = BIT(6); return 0; }
        if (scenario == 16 || scenario == 19 || scenario == 36 || event_index >= 2) { *value = 0; return 0; }
        const u8 *packet = event_index ? addr_packet : check_packet;
        if (scenario == 21) packet = event_index ? check_packet : addr_packet;
        if (scenario == 22) { /* Address alone is insufficient. */
            if (event_index) { *value = 0; return 0; }
            packet = addr_packet;
        }
        memcpy(registers+0x40, packet, 11);
        if (scenario == 2 && event_index == 1) registers[0x42] ^= 1;
        set_reg(7, PEN_INT_ASK | (scenario == 23 && event_index == 1 ? PEN_INT_ATTACH : 0));
    }
...
int main(void)
{
    ...
    puts("PASS: 37 attachment cases; protection defaults/readbacks, startup mailbox isolation, fresh ASK/checksum identity, limits, loss/deadline cutoff, TX and supply-cycle gating/cleanup");
}
'''
with tempfile.TemporaryDirectory(prefix='pen-attach-') as temp:
    path, binary = Path(temp)/'attach.c', Path(temp)/'attach'
    names = ('pen_off', 'pen_write', 'pen_write_checked', 'pen_packet', 'pen_sample',
             'pen_prepare_attach', 'pen_cycle', 'pen_cutoff', 'pen_receive', 'pen_attach')
