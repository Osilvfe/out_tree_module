...
| `caihong_pen_power.ko` | Startup address exchange validated; bounded charge diagnostic under test | Stage8d validates both startup frames and powers off cleanly. Stage9 adds charge observation after verified identity/protections; its first run timed out before permitting charge. Stage9a separates cold-start readiness from handshake timing and awaits hardware validation. Automatic charging remains unimplemented. See [`docs/cps8601-attachment.md`](docs/cps8601-attachment.md). |
