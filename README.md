...
| `caihong_pen_power.ko` | Startup identity validated; charge observation still unconfirmed | Stock 0x0118 firmware confirms TX mode 2. Stage9b's hardware run stopped earlier when the first ID ACK was followed by NACK. Stage9c retries incomplete initial identification within the existing deadline and passes 84 fault cases; its hardware test is pending. Automatic charging remains unimplemented. See [`docs/cps8601-attachment.md`](docs/cps8601-attachment.md). |
...
is installed and enabled; scan restoration, deliberate disconnect/reconnect
and automatic connection/mode restoration across a full reboot pass, including
user-confirmed drawing and pressure after reboot. Automatic wireless charging
