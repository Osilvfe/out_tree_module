...
| `oneplus_pogo.ko` | Stage6a boot/keymap confirmed; Stage6b pauses under investigation | Search/Fn and F1–F12 mappings work. Stage6b sends commands to restore the hardware target after F4 and keeps Fn+F4 as the desktop toggle. The user reports brief pauses on all keys; desktop disable-while-typing and any remaining F4 firmware interruption need separate checks. See [`docs/pogo-keymap.md`](docs/pogo-keymap.md). |
